...
> 3 API 节点 + 2 DB 节点，manager 只设 1 台（5 台不够 3 manager 做 HA，单 manager 够用）

## 一、每台机器初始化（5台都执行）

```bash
# 1. 配置 Docker IPv6
bash scripts/swarm/setup-docker-ipv6.sh
...
docker swarm join --token SWMTKN-xxx [2001:db8::1]:2377
```
...
| 类型 | Key | 示例值 |
|------|-----|--------|
| Secret | POSTGRES_PASSWORD | 你的密码 |
| Var | SWARM_DOMAIN | api.example.com |
| Var | DEPLOY_MODE | multi |
| Var | API_REPLICAS_MULTI | 3 |
