...
之前（都有 DNS 问题）:                                                                                                                                                         
init 容器跑迁移 → DNS 超时 → Pod 卡住
主容器命令跑迁移 → DNS 超时 → 探针杀掉

现在:
Helm 部署 5 个服务 → 全部 ready → kubectl exec 在已运行的 Pod 上跑迁移

- 已运行的 Pod DNS 100% 正常（cart/user/product/order 都证明了）
- 迁移不再影响 Pod 启动和探针
- api-gateway 恢复用 Dockerfile 默认 CMD，和其他服务一样干净
- 迁移失败有 3 次重试
