 ...
 *
 * 用法:
 *   bun run seed:prod                      # 正常幂等 seed
 *   bun run seed:prod -- --reset-seed      # 先清除旧 seed 数据再重新插入（ID 迁移用）
 */
...
const RESET_SEED = process.argv.includes('--reset-seed');

/**
 * 清除所有 data_source='seed' 的数据，按 FK 依赖顺序删除
 * 不影响 admin 手动管理的记录，不影响订单历史（order_items 无 FK 约束）
 */
async function resetSeedData() {
  console.log('=== 清除旧 seed 数据（--reset-seed）===\n');

  // 1. 收集 seed 商品的 ID 和 SKU ID（用于清理 Redis）
  const seedProducts = await db.execute(
    sql`SELECT id FROM product_service.products WHERE data_source = 'seed'`
  ) as any[];
  const seedProductIds = seedProducts.map((r: any) => r.id);

  const seedSkus = seedProductIds.length > 0
    ? await db.execute(
        sql`SELECT id FROM product_service.skus WHERE product_id = ANY(${seedProductIds})`
      ) as any[]
    : [];
  const seedSkuIds = seedSkus.map((r: any) => r.id);

  // 2. 按 FK 依赖顺序删除（子表 → 父表）
  if (seedProductIds.length > 0) {
    await db.execute(
      sql`DELETE FROM product_service.product_images WHERE product_id = ANY(${seedProductIds})`
    );
    await db.execute(
      sql`DELETE FROM product_service.product_categories WHERE product_id = ANY(${seedProductIds})`
    );
    await db.execute(
      sql`DELETE FROM product_service.skus WHERE product_id = ANY(${seedProductIds})`
    );
    await db.execute(
      sql`DELETE FROM product_service.products WHERE data_source = 'seed'`
    );
  }

  // 3. 删除分类（先删子分类，再删父分类）
  await db.execute(
    sql`DELETE FROM product_service.categories WHERE parent_id IS NOT NULL`
  );
  await db.execute(
    sql`DELETE FROM product_service.categories WHERE parent_id IS NULL`
  );

  // 4. 删除 seed banner
  await db.execute(
    sql`DELETE FROM product_service.banners WHERE data_source = 'seed'`
  );

  // 5. 清理 Redis 中旧 SKU 的库存 key
  if (seedSkuIds.length > 0) {
    const pipeline = redis.pipeline();
    for (const skuId of seedSkuIds) {
      pipeline.del(`stock:${skuId}`);
    }
    await pipeline.exec();
    console.log(`  清除 ${seedSkuIds.length} 个旧 SKU 的 Redis 库存 key`);
  }

  console.log(`  删除 ${seedProductIds.length} 个商品及关联数据`);
  console.log(`  删除分类、seed banner`);
  console.log('  旧 seed 数据清除完毕\n');
}

async function seedProd() {
  console.log('Production seed: inserting catalog data...\n');

  // 如果指定 --reset-seed，先清除旧数据
  if (RESET_SEED) {
    await resetSeedData();
  }
  ...
  // 一级分类
  const topCategories = [
    { id: catDigital, name: '手机数码', slug: 'digital', iconUrl: 'smartphone', sortOrder: 1 },
    { id: catComputer, name: '电脑办公', slug: 'computer', iconUrl: 'laptop', sortOrder: 2 },
    { id: catAppliance, name: '家用电器', slug: 'appliance', iconUrl: 'refrigerator', sortOrder: 3 },
    { id: catClothing, name: '服饰鞋包', slug: 'clothing', iconUrl: 'shirt', sortOrder: 4 },
    { id: catFood, name: '食品生鲜', slug: 'food', iconUrl: 'apple', sortOrder: 5 },
    { id: catBeauty, name: '美妆个护', slug: 'beauty', iconUrl: 'sparkles', sortOrder: 6 },
    { id: catBooks, name: '图书音像', slug: 'books', iconUrl: 'book-open', sortOrder: 7 },
    { id: catSports, name: '运动户外', slug: 'sports', iconUrl: 'dumbbell', sortOrder: 8 },
    { id: catHome, name: '家居家装', slug: 'home', iconUrl: 'sofa', sortOrder: 9 },
    { id: catBaby, name: '母婴玩具', slug: 'baby', iconUrl: 'baby', sortOrder: 10 },
  ];
  ...
  // 二级分类
  const subCategories = [
    { id: catPhone, parentId: getParentId('digital'), name: '手机', slug: 'phones', iconUrl: 'smartphone', sortOrder: 1 },
    { id: catEarphone, parentId: getParentId('digital'), name: '耳机', slug: 'earphones', iconUrl: 'headphones', sortOrder: 2 },
    { id: catSmartWatch, parentId: getParentId('digital'), name: '智能手表', slug: 'smart-watches', iconUrl: 'watch', sortOrder: 3 },
    { id: catLaptop, parentId: getParentId('computer'), name: '笔记本电脑', slug: 'laptops', iconUrl: 'laptop', sortOrder: 1 },
    { id: catTablet, parentId: getParentId('computer'), name: '平板电脑', slug: 'tablets', iconUrl: 'tablet', sortOrder: 2 },
    { id: catKeyboard, parentId: getParentId('computer'), name: '键盘鼠标', slug: 'keyboards', iconUrl: 'keyboard', sortOrder: 3 },
    { id: catBigAppliance, parentId: getParentId('appliance'), name: '冰箱洗衣机', slug: 'big-appliance', iconUrl: 'refrigerator', sortOrder: 1 },
    { id: catSmallAppliance, parentId: getParentId('appliance'), name: '小家电', slug: 'small-appliance', iconUrl: 'fan', sortOrder: 2 },
    { id: catKitchen, parentId: getParentId('appliance'), name: '厨房电器', slug: 'kitchen-appliance', iconUrl: 'cooking-pot', sortOrder: 3 },
    { id: catMenswear, parentId: getParentId('clothing'), name: '男装', slug: 'menswear', iconUrl: 'shirt', sortOrder: 1 },
    { id: catWomenswear, parentId: getParentId('clothing'), name: '女装', slug: 'womenswear', iconUrl: 'shirt', sortOrder: 2 },
    { id: catShoes, parentId: getParentId('clothing'), name: '鞋靴', slug: 'shoes', iconUrl: 'footprints', sortOrder: 3 },
    { id: catSnacks, parentId: getParentId('food'), name: '零食', slug: 'snacks', iconUrl: 'candy', sortOrder: 1 },
    { id: catDrinks, parentId: getParentId('food'), name: '饮料', slug: 'drinks', iconUrl: 'cup-soda', sortOrder: 2 },
    { id: catFresh, parentId: getParentId('food'), name: '生鲜', slug: 'fresh', iconUrl: 'salad', sortOrder: 3 },
    { id: catSkincare, parentId: getParentId('beauty'), name: '护肤', slug: 'skincare', iconUrl: 'droplets', sortOrder: 1 },
    { id: catMakeup, parentId: getParentId('beauty'), name: '彩妆', slug: 'makeup', iconUrl: 'palette', sortOrder: 2 },
    { id: catWashCare, parentId: getParentId('beauty'), name: '洗护', slug: 'wash-care', iconUrl: 'bath', sortOrder: 3 },
    { id: catLiterature, parentId: getParentId('books'), name: '文学', slug: 'literature', iconUrl: 'book-open', sortOrder: 1 },
    { id: catEducation, parentId: getParentId('books'), name: '教育', slug: 'education', iconUrl: 'graduation-cap', sortOrder: 2 },
    { id: catComic, parentId: getParentId('books'), name: '漫画', slug: 'comic', iconUrl: 'image', sortOrder: 3 },
    { id: catFitness, parentId: getParentId('sports'), name: '健身器材', slug: 'fitness', iconUrl: 'dumbbell', sortOrder: 1 },
    { id: catOutdoor, parentId: getParentId('sports'), name: '户外装备', slug: 'outdoor', iconUrl: 'tent', sortOrder: 2 },
    { id: catSportswear, parentId: getParentId('sports'), name: '运动服饰', slug: 'sportswear', iconUrl: 'shirt', sortOrder: 3 },
    { id: catFurniture, parentId: getParentId('home'), name: '家具', slug: 'furniture', iconUrl: 'sofa', sortOrder: 1 },
    { id: catBedding, parentId: getParentId('home'), name: '床上用品', slug: 'bedding', iconUrl: 'bed', sortOrder: 2 },
    { id: catStorage, parentId: getParentId('home'), name: '收纳', slug: 'storage', iconUrl: 'archive', sortOrder: 3 },
    { id: catMilkPowder, parentId: getParentId('baby'), name: '奶粉', slug: 'milk-powder', iconUrl: 'milk', sortOrder: 1 },
    { id: catDiaper, parentId: getParentId('baby'), name: '纸尿裤', slug: 'diapers', iconUrl: 'baby', sortOrder: 2 },
    { id: catToys, parentId: getParentId('baby'), name: '玩具', slug: 'toys', iconUrl: 'blocks', sortOrder: 3 },
  ];
  ...
  const bannerData = [
    { title: 'Spring Digital Sale', subtitle: '数码春季大促 全场低至5折', imageUrl: 'https://picsum.photos/seed/iphone-13-pro/800/800', linkType: 'category' as const, linkValue: 'digital', sortOrder: 1 },
    { title: 'iPhone 15 Pro Max', subtitle: '钛金属设计 Pro级芯片', imageUrl: 'https://picsum.photos/seed/iphone-13-pro-1/800/800', linkType: 'product' as const, linkValue: 'iphone-15-pro-max', sortOrder: 2 },
    { title: 'Fashion Week', subtitle: '时尚穿搭精选 新品上市', imageUrl: 'https://picsum.photos/seed/dress-pea/800/800', linkType: 'category' as const, linkValue: 'clothing', sortOrder: 3 },
    { title: 'Dyson V15 Detect', subtitle: '激光探测灰尘 深度清洁', imageUrl: 'https://picsum.photos/seed/dyson-v15-detect/800/800', linkType: 'product' as const, linkValue: 'dyson-v15-detect', sortOrder: 4 },
    { title: 'Best Sellers Books', subtitle: '年度畅销书单 买3免1', imageUrl: 'https://picsum.photos/seed/best-sellers/800/800', linkType: 'category' as const, linkValue: 'books', sortOrder: 5 },
    { title: 'Fresh Fruits', subtitle: '进口生鲜直达 新鲜到家', imageUrl: 'https://picsum.photos/seed/strawberry/800/800', linkType: 'category' as const, linkValue: 'fresh', sortOrder: 6 },
  ];
