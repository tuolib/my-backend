...
const logFormat =
  (process.env.LOG_FORMAT as LogFormat | undefined) ?? (isProd ? 'json' : 'pretty');
...
function write(level: LogLevel, msg: string, meta?: Record<string, unknown>): void {
  ...
  const line =
    logFormat === 'json'
      ...
      : `[${entry.time}] ${level.toUpperCase().padEnd(5)} ${requestId ? `[${requestId}] ` : ''}${msg}${meta ? ' ' + JSON.stringify(meta) : ''}`;
